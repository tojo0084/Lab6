...
// 12 grundvärden är definierade i arrayen nedan, men värdena kan också vara
// tiopotenser av dessa
// t.ex. 82000 eller 0.0082.
const long double E12_VALUES[] = {1.00, 1.20, 1.50, 1.80, 2.20, 2.70,
...
// Storleken 500 nedan räcker för en stor array med E12-värden mellan 10^-20 och
// 10^20
static long double values[500]; // det funakr INTE att skriva
                                // "maxMinusPower+maxPlusPower+1" innanför
                                // hakparentserna
static int numberOfArrayItems =
    0; // faktiska antalet element i arrayen som används

// antalet decimaler nedan är inte vetenskapligt utvalt, och om man vill kunna
...
// numberOfArrayItems till en siffra större än 0,
// vilket därför kan utnyttjas för att innan anrop kontrollera om anrop behövs.
// Funktionen kommer att populera en stor array med E12-värden tillhörande olika
// tiopotenser.
...
void initializeArrayWithE12valuesOfDifferentPowers() {
  int numberOfValuesPerE12sequence =
      sizeof(E12_VALUES) / sizeof(long double); // istället för att hårdkdoa 12
...
int
iterateArrayWithE12valuesAndTryToSumCombinationOfValues(float orig_resistance,
                                                        float *res_array) {
  // Det finns naturligtvis ENORM potential att skriva om den här funktkionen på
  // ett bättre sätt..
  ...
  while (loopCounter < 3) {
    ...
    for (int i = 0; i < numberOfArrayItems - 2; i++) {
      ...
      for (int j = i + 1; j < numberOfArrayItems - 1; j++) {
        ...
        if (fabs(orig_resistance - sum_of_two_values) <
        ...
        for (int k = j + 1; k < numberOfArrayItems; k++) {
          ...
          if (fabs(orig_resistance - sum_of_three_values) <
              DELTA_VALUE_FOR_EQUALITY) {
            res_array[0] = values[i];
            res_array[1] = values[j];
...
int e_resistance(float orig_resistance, float *res_array) {
  ...
  return iterateArrayWithE12valuesAndTryToSumCombinationOfValues(
