...
/**
E12 grundvärden är definierade i arrayen nedan, men E12-värde kan dessutom vara
tiopotenser av dessa.
...
/**
Array-storleken 500 nedan skulle räcka till för en array med E12-värden mellan
10^-20 och 10^20.
Den hårdkodas här till en större siffra än vad som behövs för att slippa ändra
på flera ställen om man vill justera intervallet, eftersom det INTE fungerar att
här allokera med konstanter på följande sätt:
...
Siffran kommer att initieras i en funktion som fyller arrayen med sina
E12-värden.
*/
static int numberOfArrayItems;

/**
Siffran anger gränsen för hur olika två flyttal får vara för att de ska
betraktas som lika.
Antalet decimaler är inte vetenskapligt utvalt, och om man vill kunna använda
minimalt små E12-värden får man nog försöka justera värdet.
*/
const long double DELTA_VALUE_FOR_EQUALITY = 0.000001;

/**
Hjälpfunktion som används som parameter (funktionspekare) till qsort för att
sortera en float array i sjunkande ordning.
För att arrayen ska bli sorterad i sjunkande ordning ska funktionen returnera
ett posiitivt tal om a är större än b.
Ett negativt tal ska returneras om b är större än a, och noll ska returneras om
talen ska betraktas som lika.
*/
...
/**
Initierar en stor array med alla tänkbara E12-värden med hjälp av en mindre
array med 12 st grundvärden mellan 1 och 10.
...
static void initializeArrayWithE12valuesOfDifferentPowers() {
  int numberOfValuesPerE12sequence =
      sizeof(E12_BASE_VALUES) /
      sizeof(long double); // istället för att hårdkdoda 12
...
/**
Loopar igenom kombinationer av E12-värden för att testa om en summering av 1-3
sådana termer resulterar i en summa lika med inparametern orig_resistance.
Om termer hittas fylls de i parameter-arrayen som således fungerar som en
output-parameter.

@param desiredSum den summa som funktionen försöker skapa genom att summera 1-3
termer som itereras
@param resultTerms ska vara en pekare till en array med plats för upp till 3
termer som summerar till desiredSum
@param potentialTermsForSumming  array med alla de termer som kan kombineras för
att försöka summera till önskvärda summan.
@return antalet termer (1-3) eller 0 om inga kombinationer av termer resulterade
i önskvärd summa.
*/
static int iterateArrayAndTryToSumCombinationOfValues(
    float desiredSum, float *resultTerms,
    long double *potentialTermsForSumming) {
  // Det finns naturligtvis ENORM potential att skriva om den här funktionen på
  // ett bättre sätt..
  // Exempel: Om man skickar in parametern desiredSum=100000 så kommer loopen
  // ändå börja med att prova summering av värden 0.001 +
  // 0.0012 o.s.v.
  // men eftersom det här inte är en algoritm-kurs så ska det inte spela någon
  // roll.
  ...
  // därför loopas först alla värden för att se om de ensamma kan summera till
  // önskat värde,
  ...
  // förklarar varför man inte kan utgå från det högsta matchande värdet och
  // sedan fortsätta med diffarna.
  while (loopCounter < 3) {
    loopCounter++;
    for (int i = 0; i < numberOfArrayItems - 2; i++) {
      if (fabs(desiredSum - potentialTermsForSumming[i]) <
          DELTA_VALUE_FOR_EQUALITY) {
        resultTerms[0] = potentialTermsForSumming[i];
        // Om inte alla 3 komponenterna behövs ska de som inte används fyllas
        // med värdet 0.
        resultTerms[1] = 0;
        resultTerms[2] = 0;
        return 1;
      ...
      for (int j = i + 1; j < numberOfArrayItems - 1; j++) {
        sum_of_two_values =
            potentialTermsForSumming[i] + potentialTermsForSumming[j];
        if (fabs(desiredSum - sum_of_two_values) < DELTA_VALUE_FOR_EQUALITY) {
          resultTerms[0] = potentialTermsForSumming[i];
        ...
        for (int k = j + 1; k < numberOfArrayItems; k++) {
          sum_of_three_values = potentialTermsForSumming[i] +
                                potentialTermsForSumming[j] +
                                potentialTermsForSumming[k];
...
/** Dokumentation för funktionen: se deklarationen i .h-filen */
int e_resistance(float orig_resistance, float *res_array) {
  ...
  return iterateArrayAndTryToSumCombinationOfValues(orig_resistance, res_array,
                                                    E12_values);
}
